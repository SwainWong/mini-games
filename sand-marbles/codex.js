(function(root,factory){const C=factory();if(typeof module==='object')module.exports=C;else root.SandCodex=C;})(globalThis,()=>{
 const entries={
 score:{name:'矿场的目标',icon:'◆',text:'同色珠子入车 +10 分，达到目标分就过关。不必全收齐！接错、漏接或被偷只失去这颗珠子的机会，不扣已经得到的分数。剩余最高可得也不够目标时，本关结束。通关后按挖沙量评 1—3 星。'},
 rock:{name:'松动的岩石',icon:'⬟',text:'挖空石头下方，岩石先摇晃约半秒，再掉落。可以砸晕盗宝人 4 秒、清除虫子；也会砸坏矿车！看清落点再动手，损坏的矿车不会接珠，并留在轨道上。'},
 rival:{name:'盗宝人',icon:'⚑',text:'他会自己挖路、抢走附近珠子，每次装袋需要一点时间。他也会累：体力低时擦汗、喝水、休息。听到快速接近的珠子，他会侧耳确认再冲刺。抓住他休息或眩晕的时机！'},
 ...
 bomb:{name:'引线炸药',icon:'✹',text:'珠子碰到圆形压力盘才会点火。沿引线燃烧约 3 秒后爆炸，虚线圈是范围：炸碎石头、晕人、除虫，也可能炸坏矿车。珠子只被推开，不被炸掉。不使用炸药也能通关。'},
 multi:{name:'单轨车队',icon:'⇆',text:'多名队友各推一辆车，但只有一条轨道。矿车不会穿过彼此或交换顺序；遇到邻车、残骸或落石会等候、转向。预留接珠的位置和时机。'},
