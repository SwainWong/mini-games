async (page) => {
  ...
  await page.setViewportSize({width:390,height:844});await page.reload();await page.waitForFunction(()=>window.sandGame);
  ...
  await page.waitForFunction(()=>sandGame.snapshot().time>=8);
  ...
  await page.reload();await page.waitForFunction(()=>window.sandGame);const reset=await page.evaluate(()=>sandGame.snapshot());if(reset.level!==1||reset.best.length||reset.collected||reset.dug||reset.soundEnabled)throw Error('Refresh did not reset');
