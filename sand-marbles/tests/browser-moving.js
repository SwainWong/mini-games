async (page) => {
  ...
  await page.addInitScript(()=>{Math.random=()=>1/4294967296;});await page.setViewportSize({width:390,height:844});await page.reload();await page.waitForFunction(()=>window.sandGame?.snapshot().assetsReady);
  ...
  await page.waitForFunction(()=>sandGame.snapshot().time>=1.25);
  ...
  return await page.evaluate(r=>window.__movingTouchChecks=r,{realTouch:true,won:{stars:won.stars,collected:won.collected},miss:{kind:lost.failure.kind,stars:lost.stars,jarX:lost.jars[0].x,homeX:lost.jars[0].homeX},awayX:away.jars[0].x,frozen:true,restart:true,sound:true,refresh:true,errors});
}
