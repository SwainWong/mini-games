...
module.exports=function replay(w,routes,{delay=0,ticks=2}={}){
  ...
  const commands=[];for(const path of routes){const r=path.slice().reverse();for(let i=1;i<r.length;i++)for(let k=1;k<=20;k++){const a=r[i-1],b=r[i];commands.push([[a[0]+(b[0]-a[0])*(k-1)/20,a[1]+(b[1]-a[1])*(k-1)/20],[a[0]+(b[0]-a[0])*k/20,a[1]+(b[1]-a[1])*k/20]]);}}
