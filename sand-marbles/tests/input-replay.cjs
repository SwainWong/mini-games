...
module.exports=function replay(w,routes,{delay=0,ticks=2,order,recoveryOrder,skipSettledColors=false,maxStep}={}){
  ...
  const build=recovery=>{commands=[];cursor=0;for(const index of (recovery&&recoveryOrder)||order||routes.map((_,i)=>i)){
    ...
    const r=routes[index].slice().reverse();for(let i=1;i<r.length;i++){const a=r[i-1],b=r[i],n=maxStep?Math.max(1,Math.ceil(Math.hypot(b[0]-a[0],b[1]-a[1])/maxStep)):20;for(let k=1;k<=n;k++)commands.push([[a[0]+(b[0]-a[0])*(k-1)/n,a[1]+(b[1]-a[1])*(k-1)/n],[a[0]+(b[0]-a[0])*k/n,a[1]+(b[1]-a[1])*k/n]]);}
  }};
