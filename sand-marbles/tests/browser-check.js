async (page) => {
  ...
  for(let number=from;number<=to;number++){
    ...
    const routes=await page.evaluate(()=>window.sandGame.routes());
    for(const route of routes){const points=route.slice().reverse();await page.mouse.move(box.x+points[0][0]*box.width/560,box.y+points[0][1]*box.height/680);await page.mouse.down();for(const p of points.slice(1))await page.mouse.move(box.x+p[0]*box.width/560,box.y+p[1]*box.height/680,{steps:12});await page.mouse.up();}
    await page.waitForFunction(()=>window.sandGame.snapshot().state!=='playing',null,{timeout:20000});
