async (page) => {
  ...
  await page.getByRole('button',{name:'选择关卡'}).click();await page.locator('[data-level="2"]').click();
  ...
  for(const route of solutions[2]){const points=route.slice().reverse();await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[point(points[0])]});for(let i=1;i<points.length;i++){const a=points[i-1],b=points[i];for(let k=1;k<=15;k++)await cdp.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[point([a[0]+(b[0]-a[0])*k/15,a[1]+(b[1]-a[1])*k/15])]});}await cdp.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});}
  ...
  await page.evaluate(r=>window.__touchChecks=r,{level:3,state:state.state,stars:state.stars,realTouch:true,noHorizontalOverflow:true});
