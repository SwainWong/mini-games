...
for(const open of [false,true])for(const seed of (open?[34,38,40,85]:[1,300,1000,100000]))for(const ticks of [1,2,3]) {
  test(`stage 12 seed ${seed}, ${open?'both optional bags':'no bags'}, input speed ${ticks}`,()=>{
    ...
    if(open){
      a.deepEqual(w.magic.completed.map(e=>e.id).sort(),w.treasures.filter(t=>t.outcome==='bomb').map(t=>t.id).sort(),e);
      a.deepEqual(w.magic.completed.map(e=>e.kind).sort(),w.treasures.filter(t=>t.outcome==='bomb').map(t=>t.outcome).sort(),e);
