(() => {
  ...
  function finish(){
    ...
    $('#result-description').textContent=won?`${world.endCause==='timeout'?'时间到':'珠子全部结算'}！最终 ${world.score} 分，过关门槛 ${world.targetScore} 分，接到 ${world.collected} 颗珠子。${world.stars<3?'试试更细的铲子，少挖一些沙，挑战三星。':'这条精细的沙径，值得三颗星。'}`:world.reason;
  ...
  function rivalDraw(){
    const r=world.rival;if(!r||!assetsReady)return;const t=world.time,dir=r.fx<0?-1:1,size=68;ctx.save();ctx.fillStyle='#52362c';roundRect(ctx,r.x-19,r.y-56,38,5,2);ctx.fill();ctx.fillStyle=r.stamina<35?'#ef9d47':'#80be77';roundRect(ctx,r.x-18,r.y-55,36*r.stamina/100,3,1);ctx.fill();ctx.restore();
    let frame=r.phase==='digging'?Math.min(7,Math.floor(r.digCycle*8)):r.phase==='reach'?8:r.phase==='lifting'?9:r.phase==='bagging'?10:r.phase==='running'?(Math.floor(t*9)%2?0:7):7;
    ...
    if(r.phase==='digging'||r.phase==='running'){const tip=r.toolPoint();ctx.save();ctx.lineCap='round';ctx.strokeStyle='#50321f';ctx.lineWidth=3.8;ctx.beginPath();ctx.moveTo(hand.x,hand.y);ctx.lineTo(tip.x,tip.y);ctx.stroke();ctx.strokeStyle='#c69a5d';ctx.lineWidth=1.4;ctx.stroke();ctx.translate(tip.x,tip.y);ctx.rotate(Math.atan2(tip.y-hand.y,tip.x-hand.x)-Math.PI/2);const g=ctx.createLinearGradient(-7,0,7,0);g.addColorStop(0,'#52636d');g.addColorStop(.45,'#e5e8dc');g.addColorStop(1,'#6b7b87');ctx.fillStyle=g;ctx.strokeStyle='#3e454b';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(-7,-5);ctx.lineTo(7,-5);ctx.lineTo(7,3);ctx.quadraticCurveTo(0,12,-7,3);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();}
  ...
  function render(){
    ...
    for(const f of floaters){ctx.save();ctx.globalAlpha=Math.min(1,f.life*2);ctx.font='bold 16px sans-serif';ctx.textAlign='center';ctx.fillStyle=f.color;ctx.strokeStyle='#462e23';ctx.lineWidth=3;ctx.strokeText(f.text,f.x,f.y);ctx.fillText(f.text,f.x,f.y);ctx.restore();}for(const b of bursts){if(b.type==='blast'){ctx.save();ctx.globalAlpha=Math.min(1,b.life*2);bombArt(ctx,3,b.x,b.y,b.radius*(2.1-b.life*.6),b.radius*(2.1-b.life*.6));ctx.restore();}ctx.save();ctx.globalAlpha=b.life;ctx.strokeStyle='#fff0a5';ctx.lineWidth=5*b.life;ctx.beginPath();ctx.arc(b.x,b.y,b.radius*(1-b.life*.6),0,Math.PI*2);ctx.stroke();ctx.restore();}
  ...
  function drainEvents(){
    for(const e of world.events){if(e.type==='cart-bump')spray(e.x,e.y,0,-1,4);if(e.type==='spill'||e.type==='bag-torn')floaters.push({x:e.x,y:e.y,life:1.3,text:e.type==='spill'?'掉出来了！':'袋子破了！',color:'#ffe6a0'});sound.play(e.type,e.count||e.speed||0);if(['collect','treasure','loss'].includes(e.type))floaters.push({x:e.x,y:e.y,life:1.4,text:e.type==='collect'?'+10':e.type==='treasure'?`+${e.points}`:(e.outcome==='stolen'?'被偷':e.outcome==='wrong-color'?'错色':'漏接')+' +0',color:e.type==='loss'?'#fff0d4':'#ffec87'});if(e.type==='loss')status('这颗 +0，已得分不扣。剩余珠子和宝物仍可争取。');if(['blast','puff','stun','cart-broken'].includes(e.type)){bursts.push({...e,radius:e.radius||25,life:1});spray(e.x,e.y,0,-1,14);}if(e.type==='collect')for(let i=0;i<6;i++)particles.push({x:e.x,y:e.y,vx:(random()-.5)*80,vy:-random()*80,life:.6,color:colors[e.color].mid,size:2});}
  ...
  window.sandGame=Object.freeze({snapshot:()=>({level:current+1,...world.snapshot(),assetsReady,paused:isPaused(),seen:[...sessionSeen],pendingIntro:[...pendingIntro],visual:{grains:particles.length,dust:dust.length,workerHeight:80,rivalHeight:64,railY:646},brush,soundEnabled:sound.enabled,audioPlayed:sound.played,best:[...best],bestSand:[...bestSand],bestScores:[...bestScores]}),levels:()=>levels.map(l=>({id:l.id,title:l.title,features:[...l.features]}))});
