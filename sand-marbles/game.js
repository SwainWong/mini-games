(() => {
  ...
  function hazardDraw(){
    ...
    for(const t of world.treasures)if(!t.opened){if(t.kind==='bag')atlasSprite(art.magic,3,2,0,t.x,t.y,48,48);else lootDraw(1,t.x,t.y,42,42);ctx.fillStyle='#fff3bc';ctx.font='bold 10px sans-serif';ctx.textAlign='center';ctx.fillText(t.kind==='bag'?'随机魔法':'+'+t.points,t.x,t.y+25);}
  ...
  function rivalDraw(){
    ...
    if(['scared','dropping'].includes(r.phase)||r.phase==='fleeing'&&!r.fleeDigging){const f=r.phase==='scared'?0:r.phase==='dropping'?1:2+Math.floor(t*12)%2;atlasSprite(art.panic,4,2,f,r.x,r.y-14,74,74,dir);if(r.bag.length&&r.phase!=='dropping')sackDraw(r);if(r.phase==='fleeing'){const tip={x:r.x+dir*25,y:r.y};ctx.strokeStyle='#ffe5ab88';ctx.lineWidth=2;for(let i=0;i<3;i++){ctx.beginPath();ctx.moveTo(r.x-dir*32,r.y-24+i*8);ctx.lineTo(r.x-dir*42,r.y-22+i*8);ctx.stroke();}}return;}
    const restFrame=SandRivalRest.frame(r);if(restFrame!==null){sackDraw(r);const p=SandRivalRest.pose(restFrame);ctx.save();ctx.translate(r.x,r.y+16);ctx.scale(dir,1);ctx.drawImage(art.rest,...p.source,p.x,p.y,p.width,p.height);ctx.restore();return;}
    let frame=(r.phase==='digging'||r.phase==='fleeing'&&r.fleeDigging)?Math.min(7,Math.floor(r.digCycle*8)):r.phase==='reach'?8:r.phase==='lifting'?9:r.phase==='bagging'?10:r.phase==='running'?(Math.floor(t*9)%2?0:7):7;
    let x=r.x-dir*8,y=r.y-46;const [hx,hy]=digHands[frame];let hand={x:x+dir*(hx/362-.5)*size,y:y+hy/362*size};
    const spriteHand={...hand};if(r.pickup){const target=r.phase==='reach'?r.pickup.ball:r.heldPoint();hand={x:target.x,y:target.y};}
    sackDraw(r);ctx.save();if(r.phase==='stun'){ctx.translate(r.x,r.y);ctx.rotate(dir*.25);ctx.translate(-r.x,-r.y);}if(!['wipe','drink','rest','listen','stun'].includes(r.phase))motionSprite(frame,x,y,size,dir);ctx.restore();
    if(r.pickup){ctx.save();ctx.lineCap='round';ctx.strokeStyle='#844cbd';ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(spriteHand.x,spriteHand.y);ctx.lineTo(hand.x,hand.y);ctx.stroke();ctx.fillStyle='#885830';ctx.beginPath();ctx.ellipse(hand.x,hand.y+3,5,4,0,0,Math.PI*2);ctx.fill();ctx.restore();}if(r.pickup?.ball.held){marble(ctx,hand.x,hand.y,r.pickup.ball.r,r.pickup.ball.color);ctx.fillStyle='#855230';ctx.beginPath();ctx.ellipse(hand.x,hand.y+5,7,3,0,0,Math.PI*2);ctx.fill();}
    if(r.phase==='digging'||r.phase==='running'||r.phase==='fleeing'&&r.fleeDigging){const tip=r.toolPoint();ctx.save();ctx.lineCap='round';ctx.strokeStyle='#50321f';ctx.lineWidth=3.8;ctx.beginPath();ctx.moveTo(hand.x,hand.y);ctx.lineTo(tip.x,tip.y);ctx.stroke();ctx.strokeStyle='#c69a5d';ctx.lineWidth=1.4;ctx.stroke();ctx.translate(tip.x,tip.y);ctx.rotate(Math.atan2(tip.y-hand.y,tip.x-hand.x)-Math.PI/2);const g=ctx.createLinearGradient(-7,0,7,0);g.addColorStop(0,'#52636d');g.addColorStop(.45,'#e5e8dc');g.addColorStop(1,'#6b7b87');ctx.fillStyle=g;ctx.strokeStyle='#3e454b';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(-7,-5);ctx.lineTo(7,-5);ctx.lineTo(7,3);ctx.quadraticCurveTo(0,12,-7,3);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();}
    if(['wipe','drink','rest','listen','stun'].includes(r.phase)){const action={wipe:0,drink:1,listen:2,stun:4,rest:5}[r.phase];ctx.save();ctx.translate(r.x,r.y-46);ctx.scale(dir,1);ctx.drawImage(art.actions,...spriteFrames[action],-32,0,64,64);ctx.restore();sackDraw(r);}
    if(r.phase==='listen'){ctx.fillStyle='#ffe79e';ctx.font='bold 20px sans-serif';ctx.fillText('!',r.x+26,r.y-28);}if(r.phase==='stun')for(let i=0;i<3;i++){ctx.fillStyle='#ffe687';ctx.font='14px sans-serif';ctx.fillText('★',r.x+Math.cos(t*3+i*2.1)*23-5,r.y-40+Math.sin(t*3+i*2.1)*5);}
  }
  function magicDraw(){
  ...
  function speechDraw(){
    if(!speech)return;const o=world.operator,present=o&&o.phase!=='flee'&&o.phase!=='gone',x=284,y=664,width=150;ctx.save();ctx.font='600 11px sans-serif';const lines=[];let line='';for(const letter of speech){if(ctx.measureText(line+letter).width>width-20){lines.push(line);line='';}line+=letter;}if(line)lines.push(line);const shown=lines.slice(0,4);if(lines.length>4)shown[3]=shown[3].slice(0,-1)+'…';const h=shown.length*14+18;
    ctx.fillStyle='#fff8e6';ctx.strokeStyle='#d6a35d';ctx.lineWidth=1.4;roundRect(ctx,x,y,width,h,10);ctx.fill();ctx.stroke();if(present){ctx.beginPath();ctx.moveTo(x+width-18,y+1);ctx.lineTo(442,654);ctx.lineTo(x+width-7,y+1);ctx.fill();ctx.stroke();}else{ctx.fillStyle='#815937';roundRect(ctx,440,680,19,24,4);ctx.fill();ctx.strokeStyle='#e8c77f';for(let k=0;k<3;k++){ctx.beginPath();ctx.moveTo(444,688+k*4);ctx.lineTo(455,688+k*4);ctx.stroke();}}
    ctx.fillStyle='#765333';ctx.textAlign='left';shown.forEach((line,i)=>ctx.fillText(line,x+10,y+17+i*14));ctx.restore();
  }
