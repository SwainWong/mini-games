(() => {
  ...
  function colorGuide(){
    const figure=document.createElement('figure');figure.className='color-guide';
    const title=document.createElement('h3');title.textContent='看颜色，送到同色矿车';
    const help=document.createElement('p');help.textContent='珠子的颜色，要和车沿、车牌的颜色相同。';
    const img=document.createElement('img');img.src=art.guide.src;img.width=1024;img.height=1024;img.alt='四种正确接法：黄色琥珀珠进黄色矿车，绿色青玉珠进绿色矿车，蓝色冰蓝珠进蓝色矿车，粉色玫瑰珠进粉色矿车。箭头分别指向同色车斗。';
    const caption=document.createElement('figcaption');caption.className='guide-colors';
    for(const [name,color] of [['琥珀','#be790a'],['青玉','#147b59'],['冰蓝','#216fa5'],['玫瑰','#a13e7c']]){const label=document.createElement('span');label.textContent=name+' → '+name;label.style.color=color;caption.append(label);}
    const rules=document.createElement('div');rules.className='guide-scoring';
    const correct=document.createElement('strong');correct.textContent='✓ 普通珠同色入车 +10';
    const wrong=document.createElement('span');wrong.textContent='错色 +0，已有分数不扣';rules.append(correct,wrong);
    figure.append(title,help,img,caption,rules);return figure;
  }
  function openCodex(first){resetInput();const ids=first?pendingIntro:Object.keys(SandCodex.entries);$('#codex-title').textContent=first?'先看懂，再开采':'图文指南 · 矿场图鉴';$('#codex-close').hidden=first;$('#codex-confirm').hidden=!first;$('#rules-dialog').dataset.first=String(first);if(first&&ids.length===1&&ids[0]!=='score')$('#codex-title').textContent='新发现 · '+SandCodex.entries[ids[0]].name;const content=$('#codex-content');content.replaceChildren();if(ids.includes('score'))content.append(colorGuide());for(const id of ids){const entry=SandCodex.entries[id],card=document.createElement('article'),h=document.createElement('h3'),p=document.createElement('p');h.textContent=entry.icon+' '+entry.name;p.className='codex-behavior';p.textContent=entry.short[0];const tip=document.createElement('p');tip.className='codex-tip';tip.textContent=entry.short[1];const detail=document.createElement('details'),summary=document.createElement('summary'),full=document.createElement('p');summary.textContent='查看详细规则';full.textContent=entry.text;detail.append(summary,full);const preview=document.createElement('canvas');preview.className='codex-art';preview.width=88;preview.height=70;preview.dataset.element=id;card.append(preview,h,p,tip);if(!first)card.append(detail);content.append(card);}$('#rules-dialog').showModal();content.scrollTop=0;drawCodexArt();}
