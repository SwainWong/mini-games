...
(function(root,factory){const api=factory();if(typeof module==='object')module.exports=api;else root.SandCore=api;})(globalThis,()=>{
  'use strict';
  const W=560,H=680,BOTTOM=564,CELL=2,GW=W/CELL,GH=BOTTOM/CELL,STEP=1/120;
  ...
  class Terrain{
    ...
    refill(rect,balls=[]){let filled=0;for(let gy=clamp(Math.floor(rect.y/2),26,GH-1);gy<Math.min(GH,(rect.y+rect.h)/2);gy++)for(let gx=clamp(Math.floor(rect.x/2),0,GW-1);gx<Math.min(GW,(rect.x+rect.w)/2);gx++){const i=gy*GW+gx,x=gx*2+1,y=gy*2+1;if(this.grid[i]||balls.some(b=>b.active&&Math.hypot(b.x-x,b.y-y)<b.r+7))continue;this.grid[i]=1;this.changes.push(i);filled++;}if(filled)this.revision++;return filled;}
  ...
  function budgets(level){const t=setupTerrain(level);for(const p of level.routes)t.path(p,24);return{three:Math.ceil(t.units*1.10),two:Math.ceil(t.units*1.60)};}
  function inside(b,z){return b.x>z.x&&b.x<z.x+z.w&&b.y>z.y&&b.y<z.y+z.h;}
  class World{
    constructor(level){this.level=level;this.terrain=setupTerrain(level);this.budget=budgets(level);this.time=0;this.started=false;this.state='playing';this.reason='';this.collected=0;this.events=[];this.quakeCount=0;this.quakeLeft=0;this.effects={wormCells:0,refilled:0,wind:0,mud:0,gate:0,magnet:0,quakes:0};this.balls=[];this.jars=level.jars.map(j=>({...j,balls:[],flash:0}));
    ...
    gateOpen(g){return !this.started||((this.time+(g.phase||0))%g.period)>=g.closed;}
    ...
    mechanisms(dt){const m=this.level.mechanics;
      for(const w of m.worms||[]){const p=this.wormPoint(w);this.effects.wormCells+=this.terrain.dig(p.x,p.y,12,'worm');}
      if(m.quake&&this.quakeCount<m.quake.count&&this.time>=m.quake.first+this.quakeCount*m.quake.interval){this.quakeCount++;this.effects.quakes++;this.quakeLeft=.8;this.events.push({type:'quake'});for(let i=0;i<this.balls.length;i++){const b=this.balls[i];if(b.active&&b.y<535){b.vx+=Math.sin(i*3.7+this.quakeCount)*32;b.vy-=70;}}for(const patch of m.quake.patches)this.effects.refilled+=this.terrain.refill(patch,this.balls);}
    ...
    collide(b){
      ...
      for(const g of this.level.mechanics.gates||[]){if(this.gateOpen(g)||b.x+b.r<g.x||b.x-b.r>g.x+g.w)continue;if(b.y+b.r>g.y&&b.y-b.r<g.y+g.h){if(b.y<g.y+g.h/2){b.y=g.y-b.r;b.vy=Math.min(0,-b.vy*.05);}else{b.y=g.y+g.h+b.r;b.vy=Math.max(0,b.vy);}this.effects.gate++;}}
    }
    step(dt=STEP){
      if(this.state!=='playing')return;if(this.started){this.time+=dt;this.mechanisms(dt);}
      for(const b of this.balls){if(!b.active)continue;let gravity=b.gravity;const m=this.level.mechanics;
        for(const z of m.winds||[])if(inside(b,z)){b.vx+=z.force*dt/Math.sqrt(b.mass);this.effects.wind++;}
        for(const z of m.muds||[])if(inside(b,z)){b.vx*=Math.exp(-4*dt);b.vy*=Math.exp(-3*dt);gravity*=.65;this.effects.mud++;}
        if(b.type==='heavy')for(const z of m.magnets||[]){const dx=z.x-b.x,dy=z.y-b.y,d=Math.hypot(dx,dy);if(d<z.range&&d>1){b.vx+=dx/d*z.force*dt;b.vy+=dy/d*z.force*dt;this.effects.magnet++;}}
      ...
      for(const b of this.balls){if(!b.active||b.y<582)continue;const j=this.jars.find(j=>Math.abs(j.x-b.x)<44);if(!j){this.end(false,'珠子漏过了罐口。把通道出口对准罐子再试试。');return;}if(j.color!==b.color){this.end(false,'珠子进入了不同颜色的罐子。留住两条路线之间的沙墙。');return;}b.active=false;j.balls.push(b.color);j.flash=1;this.collected++;this.events.push({type:'collect',x:b.x,color:b.color,count:this.collected});if(this.collected===this.total){this.end(true);return;}}
    ...
    snapshot(){return{state:this.state,collected:this.collected,total:this.total,dug:this.terrain.units,stars:this.stars,budget:this.budget,time:Math.round(this.time*10)/10,effects:{...this.effects},balls:this.balls.filter(b=>b.active).map(b=>({x:Math.round(b.x),y:Math.round(b.y),type:b.type,color:b.color})),jars:this.jars.map(j=>({color:j.color,count:j.balls.length}))};}
  }
