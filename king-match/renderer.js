...
class Renderer{
 ...
 draw(g,t){
  ...
  for(const p of this.effects){const a=t-p.t;if(a<0||a>p.life)continue;c.globalAlpha=1-a/p.life;c.save();
   ...
   else{c.translate(p.x+p.vx*a,p.y+p.vy*a+150*a*a);c.rotate(a*7);if(p.type==='shard'){const im=this.tileSprites[p.sprite];c.drawImage(im,im.width*.23,im.height*.2,im.width*.35,im.height*.35,-5,-5,10,10);}else{c.fillStyle=p.color;c.fillRect(-2,-2,4,4);}}
