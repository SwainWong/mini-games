...
class Renderer{
 constructor(canvas,assets){this.canvas=canvas;this.c=canvas.getContext('2d');this.a=assets;this.selected=-1;this.hints=[];this.hintUntil=0;this.effects=[];this.scale=1;this.resize();}
 resize(){this.scale=Math.min(devicePixelRatio||1,2);this.canvas.width=720*this.scale;this.canvas.height=HEIGHT*this.scale;}
 atlas(id,x,y,w,h){this.c.drawImage(this.a.atlas,...ATLAS[id],x,y,w,h);}
 ...
 gem(v,x,y){const c=this.c;if(v===3){c.fillStyle='#f3e5c2';c.strokeStyle='#b69a65';c.lineWidth=3;c.beginPath();c.roundRect(x+2,y+2,CELL-4,CELL-4,8);c.fill();c.stroke();this.atlas(7,x+15,y+14,CELL-30,CELL-28);}else this.atlas(v,x+2,y+2,CELL-4,CELL-4);}
 king(x,y,walking,t,hurt=false){const c=this.c;c.save();if(hurt)c.filter='brightness(1.7) sepia(.4)';if(!walking){c.translate(x+145,y-146);c.scale(-1,1);this.atlas(3,0,0,128,146);}else{const frame=walking==='stand'?0:1+Math.floor(t*8)%4,w=this.a.king.width/4,h=this.a.king.height/2;c.drawImage(this.a.king,frame%4*w,Math.floor(frame/4)*h,w,h,x-70,y-143+(walking==='stand'?0:Math.sin(t*13)*2),140,143);}c.restore();if(!walking)this.atlas(4,x,180,26,140);}
 ...
 stage(s,game,t){const c=this.c;const base=s.index*SPAN;c.save();c.translate(base,0);c.drawImage(this.a.castle,0,-150,720,HEIGHT+180);
  ...
  if(game.active===s.index&&['ready','playing','lost'].includes(game.phase))this.king(s.shield.x,FLOOR,false,t,s.time-s.lastHurt<.45);
  ...
  if(game.active===s.index){for(let i=0;i<16;i++)if(i===this.selected||this.hintUntil>t&&this.hints.includes(i)){c.lineWidth=5;c.strokeStyle=i===this.selected?'#ffffff':'#ffdc75';c.strokeRect(BX+i%4*CELL+5,BY+(i>>2)*CELL+5,CELL-10,CELL-10);}}
  c.restore();
 }
