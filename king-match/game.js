...
let lastHud=0;
function frame(ms){const t=ms/1000,delta=previous?t-previous:0;previous=t;uiTime=t;if(renderer){if(!paused)game.update(delta);for(const e of game.events.splice(0)){if(e.type==='crush'){renderer.crushFx(e.x,e.y,t);audio.play('drain');}else if(e.type==='drain'){renderer.rockFx(e.x,e.y,t);audio.play('drain');}else if(e.type==='match'){renderer.burst(e.indices,e.colors,t,e.chain);audio.play('win');}else if(e.type==='hurt'){renderer.hurtAt=t;audio.play('lose');$('status').textContent=`碰刺损失 12 HP，还剩 ${e.hp} HP！`; }else if(e.type==='rally'){audio.play('win');notify('高能爆发！国王奋力推盾 2.2 秒');}else if(e.type==='spikes'){renderer.hitAt=t;audio.play('lose');$('status').textContent='碰到尖刺了！先从底部找排石路线。';}else if(e.type==='recovery'){$('status').textContent='砖块落稳中 · 国王安全等待';}else if(e.type==='recovered'){$('status').textContent='宝石已落稳，继续消除！';}else if(e.type==='invalid')notify('交换后要横竖连成至少 3 颗');}renderer.draw(game,t);if(t-lastHud>.1){renderHud();lastHud=t;}if(lastState!==game.state){lastState=game.state;if(game.state==='won')showResult();else if(game.state==='lost')resultAt=t+.65;}if(resultAt&&t>=resultAt){resultAt=0;showResult();}if(t>toastUntil)$('toast').classList.remove('show');}requestAnimationFrame(frame);}
...
Promise.all([load('assets/royal-atlas.png'),load('assets/castle.png'),load('assets/spikes-v2.png'),load('assets/emerald-tile-v3.png'),load('assets/king-frames-v4.png'),...['single','back','hurt','idle'].map(name=>load(`assets/king-${name}-${name==='back'?'v8b':'v8'}.png`))]).then(([atlas,castle,spikes,emerald,kingFrames,single,back,hurt,idle])=>{renderer=new KingRenderer(canvas,{atlas,castle,spikes,emerald,kingFrames,single,back,hurt,idle});$('start').disabled=false;$('start').textContent='开始救援  →';loadLevel(0);requestAnimationFrame(frame);}).catch(()=>{$('start').textContent='资源未加载，请刷新重试';$('ready-description').textContent='城堡美术加载失败，请检查连接后刷新。';});
})();
