...
function loadLevel(index){
 ...
 $('ready-title').textContent=index?'绕过石梁，救下国王。':'疏通落石，守住国王的 HP。';$('ready-description').innerHTML='点一颗宝石，再点相邻宝石交换。<br><b>横向或纵向连成 3 颗</b>，才会消除。<br>宝石会下落连锁，<b>全部清空</b>才获胜。';$('status').textContent='清空全部宝石，救国王脱险。';
...
function showResult(){
 ...
 $('result').hidden=false;$('status').textContent=s.state==='won'?'宝石全部清空，国王得救！':s.reason==='jam'?'通道堵塞，请重新规划消除顺序。':'HP 归零，换个消除顺序再救一次。';
}
let lastHud=0;
function frame(ms){const t=ms/1000,delta=previous?t-previous:0;previous=t;uiTime=t;if(renderer){if(!paused)game.update(delta);for(const e of game.events.splice(0)){if(e.type==='drain'){renderer.rockFx(e.x,e.y,t);audio.play('drain');}else if(e.type==='match'){renderer.burst(e.indices,e.colors,t,e.chain);audio.play('win');}else if(e.type==='hurt'){renderer.hurtAt=t;audio.play('lose');$('status').textContent=`碰刺损失 12 HP，还剩 ${e.hp} HP！`; }else if(e.type==='rally'){audio.play('win');notify('高能爆发！国王奋力推盾 2.2 秒');}else if(e.type==='spikes'){renderer.hitAt=t;audio.play('lose');$('status').textContent='碰到尖刺了！先从底部找排石路线。';}else if(e.type==='invalid')notify('交换后要横竖连成至少 3 颗');else if(e.type==='stalled'){const n=KingMatch.remaining(game.board);notify(n<3?`只剩 ${n} 颗，无法三消，请重来`:'已无可交换组合，可重排或重来');$('status').textContent=n<3?'少于三颗无法继续消除，请重来调整顺序。':'还未清空全部宝石，试试重排或重新规划。';}}renderer.draw(game,t);if(t-lastHud>.1){renderHud();lastHud=t;}if(lastState!==game.state){lastState=game.state;if(game.state==='won')showResult();else if(game.state==='lost')resultAt=t+.65;}if(resultAt&&t>=resultAt){resultAt=0;showResult();}if(t>toastUntil)$('toast').classList.remove('show');}requestAnimationFrame(frame);}
window.kingGame=Object.freeze({snapshot:(options={})=>({...game.snapshot(),paused,selected,achievements:[...achievements],...(options.geometry?{geometry:{maxOverlap:game.overlap(),stones:game.particles.map(p=>({x:p.x,y:p.y,r:p.r})),boxes:game.boxes().map(b=>({...b}))}}:{})})});
